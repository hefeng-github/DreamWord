...
class GcodeGenerator:
    ...
    def _add_header(self):
        """添加 Gcode 头部（FluidNC 写字机风格）"""
        # 获取工作区域配置
        max_x = self.work_area_width
        max_y = self.work_area_height

        self.gcode_lines.append("; Auto-generated Gcode")
        self.gcode_lines.append(f"; Work area: {max_x}x{max_y} mm")
        self.gcode_lines.append("; Generated by Intelligent Writing Machine")
        self.gcode_lines.append("")
        # FluidNC/GRBL 写字机：只需单位、绝对定位、归零、初始抬笔
        # 不需要 3D 打印机的加热(M104/M140/M190/M109)、软限位(M208)、层高等指令
        self.gcode_lines.append("G21 ; Set units to millimeters")
        self.gcode_lines.append("G90 ; Use absolute coordinates")
        self.gcode_lines.append("$H ; Home all axes")
        self.gcode_lines.append(f"G0 Z{self.z_up:.3f} F{self.feed_rate} ; Lift pen")
        self.gcode_lines.append("")

    def _add_footer(self):
        """添加 Gcode 尾部（FluidNC 写字机风格）"""
        self.gcode_lines.append("")
        self.gcode_lines.append(f"G0 Z{self.z_up:.3f} F{self.feed_rate} ; Lift pen")
        self.gcode_lines.append(f"G0 X0 Y0 F{self.feed_rate} ; Return to origin")
        # FluidNC 无加热器，无需 M104/M140 关断；保留电机失电
