...
功能：
1. 渲染文字（中文使用 Handright，英文使用指定字体）
2. 将文字位图转化为骨架图
3. 将骨架图转化为笔画图
4. 将笔画图转化为 Gcode
5. 通过串口控制写字机（FluidNC 固件）
"""
...
from src.core import GcodePoint, Stroke, SystemConfig
    ...
    def render_text_with_handright(
        ...
        try:
            ...
            if images:
                ...
                print("警告：Handright 渲染失败，使用普通渲染")
                return self.render_text(text)

        except ImportError:
            print("警告：Handright 未安装，使用普通渲染")
            return self.render_text(text)
        except Exception as e:
            print(f"警告：Handright 渲染失败：{e}，使用普通渲染")
            return self.render_text(text)
...
class GcodeGenerator:
    """Gcode 生成器 - 将笔画转化为 Gcode 命令"""

    def __init__(
        self,
        work_area_width: float = 256.0,
        work_area_height: float = 256.0,
        z_up: float = 5.0,
        z_down: float = 0.0,
        ...
        Args:
            ...
            printer_model: 打印机型号（A1MINI, A1, P1P, X1C 等）
            layer_height: 层高（毫米）
    ...
    def _add_header(self):
        """添加 Gcode 头部"""
        # 获取工作区域配置
        max_x = self.work_area_width
        max_y = self.work_area_height
        max_z = 256  # 默认 Z 轴高度

        self.gcode_lines.append("; Auto-generated Gcode")
        self.gcode_lines.append(f"; Printer: {self.printer_model}")
        self.gcode_lines.append(f"; Layer height: {self.layer_height} mm")
        self.gcode_lines.append(f"; Nozzle temperature: {self.nozzle_temp} °C")
        self.gcode_lines.append(f"; Bed temperature: {self.bed_temp} °C")
        self.gcode_lines.append(f"; Work area: {max_x}×{max_y}×{max_z} mm")
        self.gcode_lines.append("; Generated by Intelligent Writing Machine")
        self.gcode_lines.append("")
        self.gcode_lines.append("; 设置软件限位（软限位）- 防止超出工作区域")
        self.gcode_lines.append(f"M208 X{max_x} Y{max_y} Z{max_z} ; 设置软件限位")
        self.gcode_lines.append("")
        self.gcode_lines.append("G21 ; Set units to millimeters")
        self.gcode_lines.append("G90 ; Use absolute coordinates")
        self.gcode_lines.append(f"M104 S{self.nozzle_temp} ; Set nozzle temperature")
        self.gcode_lines.append(f"M140 S{self.bed_temp} ; Set bed temperature")
        self.gcode_lines.append(f"M190 S{self.bed_temp} ; Wait for bed temperature")
        self.gcode_lines.append(f"M109 S{self.nozzle_temp} ; Wait for nozzle temperature")
        self.gcode_lines.append("")
    ...
    def _add_footer(self):
        ...
        self.gcode_lines.append("")
        self.gcode_lines.append(f"G1 Z{self.z_up} F{self.feed_rate} ; Lift pen")
        self.gcode_lines.append("G28 X Y ; Home X and Y")
        self.gcode_lines.append("M104 S0 ; Turn off nozzle")
        self.gcode_lines.append("M140 S0 ; Turn off bed")
        self.gcode_lines.append("G28 X Y ; Home X and Y again")
    ...
    def save_gcode(self, gcode: str, file_path: str):
        ...
        with open(file_path, 'w', encoding='utf-8') as f:
            f.write(gcode)
        print(f"[OK] Gcode 已保存到：{file_path}")
...
class WriterMachine:
    ...
    def write_text(
        ...
        Args:
            text: 要书写的文字
            use_handright: 是否使用 Handright 渲染手写体
            save_gcode_path: Gcode 保存路径（可选）
        ...
        if save_image_path:
            ...
            print(f"[OK] 渲染图像已保存到：{save_image_path}")
        ...
        print(f"[OK] 提取了 {len(strokes)} 个笔画")
...
class MachineController:
    ...
    def send_gcode(self, gcode: str, line_delay: float = 0.1) -> bool:
        ...
        try:
            ...
            print(f"[OK] 已发送 {len(lines)} 行 Gcode 命令")
            return True

        except Exception as e:
            print(f"发送 Gcode 失败：{e}")
            return False
