...
功能：
1. 解析 MDX 格式词典数据
2. 支持智能语义匹配（使用 Sentence Transformers）
3. 词形还原和变体识别
4. 根据语境自动消歧
5. 提供多种相似度计算方法
"""
...
from src.core import WordEntry, LookupResult
...
class WordLookup:
    ...
    def _calculate_semantic_similarity(self, context: str, entry: WordEntry) -> float:
        ...
        try:
            ...
            if entry.definitions:
                definition_parts.extend(entry.definitions[:3])  # 最多取前 3 个

            # 2. 添加例句（英文，有助于理解语境）
            if entry.examples:
                definition_parts.extend(entry.examples[:2])  # 最多取前 2 个例句
    ...
    def calculate_similarity(self, context: str, entry: WordEntry,
        ...
        Args:
            ...
            use_tfidf: 是否使用 TF-IDF 加权
            use_examples: 是否考虑例句相似度
            use_ngram: 是否使用 n-gram 匹配
        ...
        if use_semantic and self.use_semantic_search:
            ...
            if semantic_score > 0:
                scores.append(('semantic', semantic_score, 0.35))  # 从 60% 降到 35%
    ...
    def find_best_match(self, entries: List[WordEntry], context: str,
        ...
        Returns:
            如果 return_scores=False: 返回最佳匹配的条目
            如果 return_scores=True: 返回 (最佳条目，所有条目分数列表)
        """
        if not entries:
    ...
    def lookup(self, word: str, context: str = "") -> LookupResult:
        ...
        # 如果 best_entry 没有释义或释义为空，使用 base_entry 的释义
        ...
        if context and len(all_definitions) > 1:
            ...
            # 如果 best_entry 没有英文释义和例句，使用 base_entry 的
        ...
        # 构建结果
        # 如果 best_entry 没有例句，使用 base_entry 的例句
